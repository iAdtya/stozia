...
Fixes applied (no DB or external deps needed — run with plain `python`):
  ...
  2. Normalize each item to ONE quantity (the max across its supplier rows),
     so both suppliers for an item are compared at the same quantity. Without
     this, a 20-piece quote always looks cheaper than a 1000-piece quote and
...
FIELDS = [
    "item_name", "manufacturer_part_number", "description", "quantity",
